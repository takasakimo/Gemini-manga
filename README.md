...
- **🤖 フルオート生成**: ①テキスト用プロンプトで構成JSONを作る → ②貼り付けて反映 → **③Gemini画像用プロンプト**をその場で表示（**API キー不要**・Cloud 対応）
...
- **🤖 フルオート**: テーマ入力 → ①構成用プロンプトをコピーして Gemini（テキスト）に貼る → ②返答JSONを「設定に反映」→ **③に表示されるプロンプトを Gemini 画像生成に貼る**（「漫画生成」タブのプロンプトと同じ）
- **📄 漫画生成（手動）**:
