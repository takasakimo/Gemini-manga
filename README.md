...
- **🖼 テーマから画像プロンプト**: テーマと画風を選ぶだけで、**Gemini 画像生成にそのまま貼るプロンプト**を表示（JSON 不要・**API キー不要**・Cloud 対応）
...
- **🖼 テーマから画像プロンプト**: テーマ入力 → 「画像用プロンプトを表示」→ 出てきた【ブロック】を順に Gemini 画像生成へコピペ（細かく編集したいときは「漫画生成」タブ）
- **📄 漫画生成（手動）**:
